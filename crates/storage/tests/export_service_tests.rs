...
use ez_booth_storage::export::{
    generate_booth_backup_filename, generate_full_backup_filename, ExportError, ExportService,
...
async fn test_export_all_collects_all_records_and_serializes() {
    ...
    assert_eq!(serialized.file_name, generate_full_backup_filename(backup.created_at));
...
async fn test_export_booth_filters_to_requested_booth() {
    ...
    assert_eq!(
        ...
        generate_booth_backup_filename(&backup.booth.id, &backup.booth.description, backup.created_at)
    );
    assert!(serialized.json.contains("Spring Market 2026"));
