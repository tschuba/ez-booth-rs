...
- [x] 2.1 In `crates/ez-booth-ui/src/pages/booth_list.rs`: In `handle_delete_booth` den Aufruf `state.booth_repository.delete(&booth.id).await` durch `state.archive_service.delete_booth_with_cascade(&booth.id).await` ersetzen
